...
elseif ($CurrentValue -eq 1) {
    Write-Host "Value is Enabled: First Time ($CurrentValue)." -ForegroundColor Green
    $Status = "COMPLIANT"
}
elseif ($CurrentValue -eq 2) {
    Write-Host "Value is Enabled: Always ($CurrentValue)." -ForegroundColor Green
    $Status = "COMPLIANT"
}
else {
    Write-Host "Value is incorrect ($CurrentValue). Expected: 1 (First Time) or 2 (Always)." -ForegroundColor Red
    $Status = "NON-COMPLIANT"
}
